defmodule Share.PageChannel do
  ...
  def handle_in("open_fav_notice", %{"id" => id}, socket) do
    user = socket.assigns.user
    user = Repo.get!(User, user.id)
    changeset = Ecto.Changeset.change(user, fav_id: id)
  ...
  def handle_in("open_follow_notice", %{"id" => id}, socket) do
    user = socket.assigns.user
    user = Repo.get!(User, user.id)
    changeset = Ecto.Changeset.change(user, follow_id: id)
    case Repo.update(changeset) do
      {:ok, user} ->
        res = %{
          fav: user.fav_id,
  ...
  def handle_in("open_address_notice", %{"id" => id}, socket) do
    user = socket.assigns.user
    user = Repo.get!(User, user.id)
    changeset = Ecto.Changeset.change(user, post_address_id: id)
    case Repo.update(changeset) do
      {:ok, user} ->
        res = %{
          fav: user.fav_id,
